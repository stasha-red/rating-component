...
export class LitRating extends LitElement {
  ...
  render() {
    const isInactiveOrDisabled = this.state === RatingState.Disabled || this.state === RatingState.Inactive;
    return html`
      <fieldset 
        class="rating rating--${this.size} ${this.state === RatingState.Inactive ? 'rating--inactive' : ''}" 
        ?disabled=${isInactiveOrDisabled}
        title=${isInactiveOrDisabled ? this.value + ' / ' + this._startsAmount : ''}
        @change=${this.onchange}
      >
        ${map(range(this._startsAmount), (index: number) => {
          return html`
            <input class="rating__input" type="radio" id="star${index + .5}" name="rating" value="${index + .5}" ?checked=${index + .5 === this.value}/>
            <label class="rating__label rating__label--left" for="star${index + .5}" title=${isInactiveOrDisabled ? '' : (index + .5) + ' / ' + this._startsAmount}>
            ...
            <input class="rating__input" type="radio" id="star${index + 1}" name="rating" value="${index + 1}" ?checked=${index + 1 === this.value}/>
            <label class="rating__label rating__label--right" for="star${index + 1}" title=${isInactiveOrDisabled ? '' : (index + 1) + ' / ' + this._startsAmount}>
